...
\begin{ques} What is the formula for the area of a triangle?
\end{ques}
\QM

Using merely the formula for the area of a triangle, we (meaning you)
will explain why the following important theorem is true. Note,
throughout this discussion we will use the convention that when we
write $AB$ we mean the \textit{length} of the segment $AB$.


\begin{thm}[Parallel-Side] 
...
\begin{ques} Compute and simplify both of the following expressions:
\[
...
\begin{ques} How can you conclude that: 
\[
\frac{AB}{AD} = \frac{AC}{AE}
\]
\end{ques}
...
\begin{thm}[Split-Side]\index{Split-Side Theorem}\index{Theorem!Split-Side} 
Given:
\[
\includegraphics{../graphics/split1.pdf}
\]
...
\begin{ques} What can you conclude about points $C$ and $C'$?
\end{ques}
\QM
...
\begin{thm}[SAS-Similarity Theorem]
...
\begin{ques} What does this mean, ``up to similarity?''
\end{ques}
\QM
...
\begin{ques} What can multiplication mean? 
\end{ques}
\QM
