...
\begin{con}[Bisecting a Segment]\index{coordinate geometry!bisecting a segment} 
...
\begin{con}[Parallel through a Point]\index{coordinate geometry!parallel through a point} 
...
\begin{con}[Perpendicular through a Point]\index{coordinate geometry!perpendicular through a point}  
...
\begin{con}[Line between two Points]\index{coordinate geometry!line between two points} 
...
\begin{con}[Intersection of a Line and a Circle]\index{coordinate geometry!intersection of a line and a circle} 
...
\begin{con}[Bisecting an Angle]\index{coordinate geometry!bisecting an angle} 
...
\begin{con}[Intersection of Two Circles]\index{coordinate geometry!intersection of two circles} 
...
\begin{ques}\index{coordinate geometry!equilateral triangle} 
...
\item Denote the set of numbers constructible by coordinate
  ...
  call $\D$ the set of \textit{Descartes numbers}\footnote{Be warned,
...
\begin{ques} Exactly what numbers are in $\C$?
\end{ques}
...
\begin{con}[Multiplication]\index{compass and straightedge!multiplication}\index{similar triangles} 
...
\begin{con}[Division]\index{compass and straightedge!division}\index{similar triangles} 
...
\begin{dfn}\index{field} A \textbf{field} is a set of numbers, which we will call $F$, that is closed under two associative and commutative operations $+$ and $\cdot$ such that:
\begin{enumerate}
\item 
\begin{enumerate} 
...
\begin{dfn}\index{closed} A set $F$ is \textbf{closed} under an operation $\star$ if for all $x,y\in F$, $x\star y\in F$.   
\end{dfn}

\begin{eg} The set of integers, $\Z$, is closed under addition, but is not closed under division.
\end{eg}

\begin{dfn}\index{associative} An operation $\star$ is \textbf{associative} if for all $x$, $y$, and $z$
\[
x\star(y \star z) = (x \star y) \star z.
\]   
\end{dfn}

\begin{dfn}\index{commutative} An operation $\star$ is \textbf{commutative} if for all $x$, $y$
\[
x\star y  =  y \star x.
\]   
\end{dfn}
...
\begin{ques} Is $\Z$ a field?  Is $\Q$ a field?  Can you think of other fields?  What about the set of constructible numbers $\C$? What about the folding and tracing numbers $\F$? What about the Descartes numbers $\D$?
\end{ques}
\QM
...
\begin{con}[Square-Roots] Start with given segments of length $a$ and $1$:
\begin{enumerate}
...
\begin{ques} Can you explain why the above theorem is true? Big hint: What is the relationship between $\C$ and $\D$?
\end{ques}
\QM
...
we will spare you gentle reader---for now\footnote{The most accessible
  discussion of this fact that I know of can be found in
  \cite{coruant}.}.

\begin{ques} Which of the following numbers are constructible?
\[
...
\begin{ques} Why does doubling the cube boil down to constructing a solution to the equation $x^3-2=0$?
\end{ques}
\QM
...
\begin{ques} Can you think of any angles that can be trisected 
...
\begin{con}[Solving Cubic Equations]\index{folding and tracing!solving cubic equations}\index{cubic equations!folding and tracing} We wish to solve equations of the form:
\[
x^3 + ax^2 + bx + c = 0
\]
\begin{enumerate}
...
\begin{ques} How do we get the ``solution'' from the slope?
\end{ques}
\QM
